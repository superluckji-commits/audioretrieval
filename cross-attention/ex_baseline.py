...
def get_config():
    return {
        "trial": "audiocaps_mixed_v6_CA",
        "output_dir": "/home/xjii0026/ml23/xjii0026/my_work/code/author/cross-attention/outputs",
...
def run():
    ...
    # ✅ 新增：model配置
    model_config = {
        'dataset': config['train'],      # 或 'audiocaps'
        'enable_a2t': True,         # 启用Audio-to-Text attention
        'enable_t2a': True,         # 启用Text-to-Audio attention
        'use_pe': False,
        'return_audio_tokens': True 
    }
    model = model_utils.init_model(model_config)
    print(model)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        val_state = model_utils.test(model, val_dl, objective, use_cross_attention=False)
        val_results.append(val_state)

        test_state = model_utils.test(model, test_dl, objective, use_cross_attention=False)
